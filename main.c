...
SDL_Rect Render(GameTexture *current, int x, int y)
{ // Render texture and display at position [x,y]
    // SDL_Rect rect = CreateRect(x, y, current->tWidth, current->tHeight);
    SDL_Rect rect = {x, y, current->tWidth, current->tHeight};
